...
import logging
import time
from dataclasses import  dataclass # ED
...
class DisFeeder(Protocol):
    def __call__(self, engine: Engine, batch: dict[str, Tensor],# | None, 
fake: Tensor) -> dict[str, Tensor]:
        ...
...
#    _ = KW_ONLY
    ...
    load_G: EngineLoader
    feed_G: GenFeeder
    load_D: EngineLoader  = None
    feed_D: DisFeeder = None
    ...
    device: str = "cuda"
    eval_fn: EvalFn  = None
#    gan_training_start_step: int  = None
    ...
    @classmethod
    def get_running_loop_global_step(cls) -> int :
        if loop := cls.get_running_loop():
            return loop.global_step
        return None

    @classmethod
    def get_running_loop_viz_path(cls, name: str, suffix: str) -> Path :
        if loop := cls.get_running_loop():
